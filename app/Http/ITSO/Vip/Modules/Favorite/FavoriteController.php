...
class FavoriteController extends BaseController
{
    ...
    public function viewAction($id)
    {
        ...
        $q = $this->pdo()->query("SELECT product.*,
         ...
         where product.id in(SELECT user_favorite.product_id FROM `user_favorite` where favorite_category_id =" . intval($id) . ")");
        $products = [];
        $productsId = [];
        $nbProductLinkClick = [];
        $productsLink = [];
 ...
 FROM user_product
  ...
  where product_id in (" . implode(',', $productsId) . ")");
    ...
    public function listByUserAction()
    {
        $url = URL;
        $app = $this->application;
        $q = $this->pdo()->query("SELECT user_favorite_category.* FROM user_favorite_category where user_id = " . intval($app->user()->getAttribute('id')) . " AND user_favorite_category.id not in (SELECT favorite_category_id FROM user_favorite where product_id = " . intval($_REQUEST['product_id']) . ") order by user_id");
        $nb = $q->rowCount();
        $form = "";
