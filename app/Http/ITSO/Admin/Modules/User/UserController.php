...
//-- penser à vérifier si l'email existe déjà
	...
	public function connectAction($id) {
		$url = URL;
		$app = $this->application;
		$admin = $this->pdo()->query("SELECT * FROM user where user_type_id = 1 AND id = " . $app->user()->getAttribute('id'))->fetch(\PDO::FETCH_ASSOC);
		$user = $this->pdo()->query("SELECT * FROM user where user_type_id = 4 AND id = " . $id)->fetch(\PDO::FETCH_ASSOC);


		$app->user()->setAttribute('id', $user['id']);
		$app->user()->setAttribute('last_name', $user['last_name']);
		$app->user()->setAttribute('first_name', $user['first_name']);

		/** @var \Http\Itso\Front\FrontApplication $front */
		global $front;
		redirect($front->router()->getRoute('front_home'));
	}
